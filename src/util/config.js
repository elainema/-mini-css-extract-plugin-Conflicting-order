let config =  {
    api_host: process.env.NODE_ENV !== 'production' ? '172.30.10.213' : '172.30.10.213',
    app_name: 'newymapi',
    ...
    swfupload: {
        upload_url: {
            ...
            uploadCertificate: "/Upload/uploadCertificate"
        },
        flash_url: 'asset/js/swfupload/swfupload.swf'
    },
    defaultSrc:'http://uploads.yeahmobi.com/',
    defaultImg:'/img/default_offer_image.png',
    ajax_loader_img:require('@/assets/image/ajax_loader.gif'),
...
if (config.api_host) {
    if (config.api_ssl) {
        ...
        config.swfupload.upload_url.uploadCertificate = 'http://' + config.api_host + "/" + config.app_name + config.swfupload.upload_url.uploadCertificate;
