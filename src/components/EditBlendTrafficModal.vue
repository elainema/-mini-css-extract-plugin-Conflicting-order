<template>
<div id="" class="modal  fade" aria-hidden="false" ref="modal" id="editBlendTraffic">
    ...
    <div class="modal-dialog" style="width:900px">
      ...
      <div class="modal-header"> <a class="close offline_cancel" @click.prevent="hideModal" data-dismiss="modal">×</a>
            ...
            <div class="modal-footer">
                <button class="btn btn-default " @click.prevent="resetData">Reset</button>
...
<script>
import Vue from "vue";
import { Select, Option, Table, TableColumn, OptionGroup } from 'element-ui'
  ...
  methods: {
    filterMethod: function(data){
        if (this.aff_id == ""){
            ...
            return data.aff_id == this.aff_id
        }
    },
        ...
        if (allRate < 1 && allRate > 0){
            if (emptyShadowObj.length == 0){
                ...
                /*if (this.aff_id == ""){ //没有search
                    ...
                    this.searchData.splice(lastAffiliateIndex.pop()+1,0,{  
                        "offer_id": this.$route.query.id,
                        "aff_id": o.aff_id,
                        "shadow_aff_id": "",
                        "click": o.click,
    ...
    getLastDayAffDataByOffer(){
      ...
      mergeData(){
          let that = this
            let resultArray = []
            let lastDayKey = []
            resultArray = _.cloneDeep(that.lastDayData)
    ...
    getInitData() {
        ...
        Vue.http.get("Blend/getShadowAffs").then(response => {
            ...
            //that.shadowChosen.unshift({'value': '', 'text': '- -'})
        }, response => {
            // error callback
            that.showAlert(response.body.msg);
        })
