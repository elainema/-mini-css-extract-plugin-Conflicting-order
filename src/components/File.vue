...
<div :class="loadingZone ? 'ajax_loading' : '' ">
    <div class="box-content form-horizontal frm-entity frm-detail" id="creative_files">
        ...
        <span class="btn btn-file" :disabled="offer_file_language == ''">
            {{btnName}}<input type="file"  data-control-name="bt-button"  disabled="" :class="'btn btn-primary'" ref="btn_upload" accept=".jpg,.jpeg,.png,.gif,.bmp,.txt,.doc,.ppt,.pdf,.zip,.mp4" v-if="multiple" multiple="multiple">
            <input type="file"  data-control-name="bt-button"  disabled="" :class="'btn btn-primary'" ref="btn_upload" accept=".jpg,.jpeg,.png,.gif,.bmp,.txt,.doc,.ppt,.pdf,.zip,.mp4" v-else>
        </span>
...
<script>
import confirmWnd from './ConfirmWnd.vue'
...
export default {
    ...
    mounted () {
        let that = this
        that.cookie_hack = that.cookieHash
        /*if (that.swfuSettings) {
            window.swfu = new SWFUpload(that.swfuSettings);
        }*/
        ...
        $btn_upload.on("change", function(e) {
            ...
            if(this.files.length) {
                _.each(this.files, function(file){
                    that.onUploadFileChange(file, $btn_upload)
                })
                $btn_upload.val("")
